 ...
 *
 * Only the two SimulTrans blues, so every swirl lands somewhere on a blue
 * gradient. The v1 sunset/ember/royal dyes are gone with the rest of the
 * pre-rebrand palette.
 */
// #68AEE0 light blue — the dominant dye, what most swirls read as.
const AZURE: RGB = { r: 0.16, g: 0.34, b: 0.5 };
// #00529B primary blue — the deeper tone the light blue settles into.
const DEEP: RGB = { r: 0.04, g: 0.16, b: 0.32 };
// A lifted tint of the light blue, standing in for v1's highlight dye.
const HIGHLIGHT: RGB = { r: 0.26, g: 0.45, b: 0.6 };

/** ~70% azure, ~20% deep, ~10% highlight. Never white, never warm. */
function pickColor(): RGB {
  const n = Math.random();
  if (n < 0.7) return { ...AZURE };
  if (n < 0.9) return { ...DEEP };
  return { ...HIGHLIGHT };
}
...
export function FluidCanvas() {
  ...
  useEffect(() => {
    ...
    const wake = setTimeout(() => {
      sim.splatNorm(0.5, 0.55, 0, 220, scale(AZURE, 1.1));
    }, 500);
